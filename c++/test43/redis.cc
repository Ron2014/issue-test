 ...
 * @Author: Ron
 * @LastEditors: Ron
 * @FilePath: \issue-test\c++\test43\redis.cc
 */

/*
EVLOOP_ONCE | EVLOOP_NONBLOCK

 gcc -o test  asyn.c -lhiredis -levent -lpthread -Wformat
...
int main (int argc, char **argv) {
  ...
  redisAsyncCommand(c, getCallback, (char*)"auth 123456", "auth 123456");//发送get命令
  redisAsyncCommand(c, getCallback, "set aaa %b", "bbb", sizeof("bbb"));//发送set命令
  redisAsyncCommand(c, getCallback, (char*)"GET aaa", "GET aaa");//发送get命令

  bool bRet = true;
  while(bRet)
  {
        event_base_loop(base, EVLOOP_ONCE | EVLOOP_NONBLOCK); //开始libevent循环。注意在这一步之前redis是不会进行连接的
  }
  return 0;
}